...
public class NagarebosiMover : MonoBehaviour
{
    ...
    float span = 20f; //値の数字秒ごとに隕石が飛ぶ
    float currentTime = 0f;
    float keizokuCurrentTime = 0f;
    float keizokuZikan = 5f;
    ...
    void Update()
    {
        ...
        if (move == true)
        {
            //keizokuCurrentTime += Time.deltaTime;
            float step = speed * Time.deltaTime;
            this.gameObject.transform.position = Vector3.MoveTowards(transform.position, endPos, step);

            //if (keizokuCurrentTime < keizokuZikan && this.gameObject.transform.position == endPos)
            //    MoveInseki();
    ...
    void MoveInseki()
    {
        ...
        float roatationZ;
        if (moveFromRight == true) roatationZ = 45f + angle;
        else roatationZ = 45f + angle;
